...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        self.encoder = Encoder(
            [
                ...
                ) for l in range(configs.e_layers)  # e_layer:Num of encoder layers (defaults to 2)
            ],
        ...
        self.ada_pool = nn.AdaptiveAvgPool1d(configs.seq_len)   # 自适应平均池化
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec,
        ...
        delta = self.delta_learner(x_raw, mean_enc)      # B x S x E, B x 1 x E -> B x S(eq)
        ...
        # De-normalization
        assert self.target in ['x', 'y', 'z']
        types_map = {'x': 0, 'y': 1, 'z': 2}
        set_types = types_map[self.target]
        if self.features == 'M':
            dec_out = dec_out * torch.cat([std_enc[:, :, :3], std_enc[:, :, -3:]], dim = 2)\
                      + torch.cat([mean_enc[:, :, :3], mean_enc[:, :, -3:]], dim = 2)
        elif self.features == 'S':
            dec_out = dec_out * torch.cat([std_enc[:, :, 0].unsqueeze(2), std_enc[:, :, -1].unsqueeze(2)], dim = 2) \
                      + torch.cat([mean_enc[:, :, 0].unsqueeze(2), mean_enc[:, :, -1].unsqueeze(2)], dim = 2)
        else:
            dec_out = dec_out * torch.cat([std_enc[:, :, set_types].unsqueeze(2), std_enc[:, :, set_types + 5].unsqueeze(2)], dim = 2) \
                      + torch.cat([std_enc[:, :, set_types].unsqueeze(2), std_enc[:, :, set_types + 5].unsqueeze(2)], dim = 2)
